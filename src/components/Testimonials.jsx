      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto space-y-4 mb-16">
          ...
          <p className="text-slate-300 text-sm sm:text-base leading-relaxed">
            See what landlords, real estate investors, and property owners say about our management standard and rent remittance consistency.
          </p>
        </div>
              ...
              <SwiperSlide key={idx}>
                <div className="glass-card p-8 h-full flex flex-col justify-between border-gold-glow relative">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex space-x-1 text-gold-400">
                        {[...Array(test.rating)].map((_, sIdx) => (
                          <Star key={sIdx} className="w-4 h-4 fill-gold-400" />
                        ))}
                      </div>
                  ...
                  <div className="pt-6 mt-6 border-t border-white/5 flex items-center justify-between">
                    <div>
                      <h4 className="font-serif font-bold text-white text-base">{test.name}</h4>
                      <p className="text-[11px] text-gold-400 font-medium">{test.role}</p>
                    </div>

                    <div className="flex items-center text-[10px] text-slate-400 bg-obsidian-900 px-2.5 py-1 rounded-full border border-white/10">
                      <MapPin className="w-3 h-3 text-gold-500 mr-1" />
